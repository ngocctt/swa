...
public class SalesOrderDetailImpl {
	...
	public SalesOrder getByOrderNo(String orderNo) {
		String sql = "SELECT customer.customer_id, " + "customer.first_name as customer_first_name, "
				+ "customer.last_name as customer_last_name," + "sp.sales_person_id, "
				+ "sp.first_name as sp_first_name, " + "sp.last_name as sp_last_name, " + "so.order_no, "
				+ "so.order_date, " + "so.overdue_date, " + "so.order_disc, " + "so.tax_amt, " + "so.total_amt, "
				+ "so.payment, " + "so.description " + "FROM sales_people sp "
				+ "JOIN sales_orders so ON sp.sales_person_id = so.sales_person_id "
				+ "JOIN customers customer ON so.customer_id = customer.customer_id " + "WHERE so.order_no = :orderNo";
		MapSqlParameterSource parameterSource = new MapSqlParameterSource();
		parameterSource.addValue("orderNo", orderNo);
		List<SalesOrder> salesOrderList = namedParameterJdbcTemplate.query(sql, parameterSource,
				(resultSet, i) -> toSalesOrder(resultSet));
	...
	public List<SalesOrderDetail> getAllByOrderNo(String orderNo) {
		String sql = "SELECT sod.order_no, " + "sod.qty, sod.price, " + "sod.item_disc, " + "sod.tax_amt, "
				+ "sod.amount, " + "item.item_id, " + "item.item_name, " + "item.unit "
				+ "FROM sales_order_details sod " + "JOIN items item ON sod.item_id = item.item_id "
	...
	private SalesOrderDetail toSalesOrderDetail(ResultSet rs) throws SQLException {
		...
		Item item = Item.builder().itemId(rs.getString("item_id")).itemName(rs.getString("item_name"))
	...
	private SalesOrder toSalesOrder(ResultSet rs) throws SQLException {
		...
		Customer customer = Customer.builder().customerId(rs.getString("customer_id"))
				.firstName(rs.getString("customer_first_name")).lastName(rs.getString("customer_last_name")).build();
		SalesPerson salesPerson = SalesPerson.builder().salesPersonId(rs.getString("sales_person_id"))
				.firstName(rs.getString("sp_first_name")).lastName(rs.getString("sp_last_name")).build();
	...
	public Specification<SalesOrder> getSalesOrderSpec() {
